<?php
// register_patient.php - Register new patient (Updated for new database)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first_name) || empty($last_name) || empty($gender) || empty($bdate) || empty($username) || empty($password)) {
        ...
        try {
            ...
            // Insert into tblinfo (specialization empty for patients)
            $stmt = $pdo->prepare("INSERT INTO tblinfo (user_id, first_name, last_name, middle_name, specialization, bdate, gender, contact, address, image) 
                                   VALUES (?, ?, ?, ?, '', ?, ?, ?, ?, ?)");
            $stmt->execute([$user_id, $first_name, $last_name, $middle_name, $bdate, $gender, $contact, $address, $image]);
            ...
            $pdo->commit();
            $success = "Patient registered successfully! User ID: $user_id";
...
<head>
    ...
    <title>Register Patient - AppointmentEase</title>
        ...
        <div class="max-w-3xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-gray-600">Add a new patient to the system</p>
                </div>
                <a href="users_list.php" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">
